...
const SearchResults = () => {
  ...
  const { filteredProducts, allCategories, allColors, allSizes } = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      // For relevance, we prioritize exact matches, then partial matches
      if (searchTerm) {
        ...
        if (aCategory === searchTermLower) return -1;
        if (bCategory === searchTermLower) return 1;
        if (aCategory.startsWith(searchTermLower)) return -1;
        if (bCategory.startsWith(searchTermLower)) return 1;
      }
      return 0;
    });

    // Get unique categories, colors, and sizes for filters
    const categories = ['all', ...new Set(products.filter(p => p.status === 'approved').map(p => p.category))];
    const colors = [...new Set(products.filter(p => p.status === 'approved' && p.color).map(p => p.color))];
    const sizes = [...new Set(products.flatMap(p => p.status === 'approved' && p.sizes ? p.sizes : []))];
  ...
  }, [products, searchTerm, filterOptions, sortOrder]);
  ...
  const toggleCompareProduct = (product) => {
    setCompareProducts(prev => {
      ...
      if (exists) {
        ...
        // In a real implementation, we might want to show a message or replace the first item
        return [...prev.slice(1), product]; // Replace the first item with the new one
      }
    });
  };
  ...
  // Save search term to localStorage for search history
  useMemo(() => {
    if (searchTerm) {
      // Get existing search history
      const searchHistory = JSON.parse(localStorage.getItem('searchHistory') || '[]');
      
      // Add current search term if it's not already in history
      if (!searchHistory.includes(searchTerm)) {
        const updatedHistory = [searchTerm, ...searchHistory].slice(0, 10); // Keep only last 10 searches
  ...
  return (
    <div className="container mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8">
      <div className="mb-4 sm:mb-6">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 mb-2">Search Results</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 lg:gap-8">
        ...
        <div className="w-full lg:w-1/4">
          <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 lg:sticky lg:top-20">
            ...
            {/* Price Filter */}
            <div className="mb-4 sm:mb-6">
              <h3 className="font-semibold text-gray-700 mb-2 sm:mb-3 text-sm sm:text-base">Price Range</h3>
            ...
            {/* Category Filter */}
            <div className="mb-4 sm:mb-6">
              <h3 className="font-semibold text-gray-700 mb-2 sm:mb-3 text-sm sm:text-base">Category</h3>
              <select
                name="category"
